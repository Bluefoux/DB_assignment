...
if [ -f /app/nginx.conf ]; then
    ...
    content_server=$content_server"    listen ${USE_LISTEN_PORT} ssl;\n"
    content_server=$content_server"    listen [::]:${USE_LISTEN_PORT} ssl;\n"
    content_server=$content_server"    ssl_certificate /root/ssl/cert.pem;\n"
    content_server=$content_server"    ssl_certificate_key /root/ssl/key.pem;\n"
	...
	content_server=$content_server"    server_name http://localhost;\n"
    content_server=$content_server'    location / {\n'
    content_server=$content_server'        include uwsgi_params;\n'
    content_server=$content_server'        uwsgi_pass unix:///tmp/uwsgi.sock;\n'
    content_server=$content_server'    }\n'
    content_server=$content_server"    return ${USE_LISTEN_PORT};\n"
    content_server=$content_server'}\n'
